...
func (s *bitcoinWallet) SignTransaction(
	...
	for i, input := range updater.Upsbt.UnsignedTx.TxIn {
		...
		if !bytes.Equal(utxo.PkScript, tree.ANCHOR_PKSCRIPT) {
	...
	for i, input := range ptx.Inputs {
		if len(input.TaprootLeafScript) > 0 {
			if err := s.signTapscriptSpend(updater, input, i, txsighashes, prevoutFetcher, ptx.UnsignedTx); err != nil {
				return "", err
			}
			continue
		}
		...
		if input.WitnessUtxo != nil {
			...
			if bytes.Equal(input.WitnessUtxo.PkScript, onchainPkScript) {
				updater.Upsbt.Inputs[i].TaprootInternalKey = schnorr.SerializePubKey(s.walletData.PubKey)
		...
		if len(input.TaprootInternalKey) > 0 {
			if err := s.signTaprootKeySpend(updater, input, i, txsighashes, prevoutFetcher, ptx.UnsignedTx); err != nil {
				return "", err
			}
			continue
		}
...
func (w *bitcoinWallet) signTapscriptSpend(
	...
	for _, leaf := range input.TaprootLeafScript {
		...
		if sign {
			...
			if !sig.Verify(preimage, w.walletData.PubKey) {
...
func (w *bitcoinWallet) signTaprootKeySpend(
	...
	xOnlyPubkey := schnorr.SerializePubKey(w.walletData.PubKey)
